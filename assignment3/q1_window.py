...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    for sentence, labels in data:
        ...
        for i in range(len(sentence)):
            window_word = []
            if (i-window_size) >= 0 and (i + window_size) <= (len(sentence)-1):
                window_word = [feature for word in sentence[i-window_size:i + window_size + 1] for feature in word]
            else:
                for j in range(-window_size, window_size + 1):
                    indx = i + j
                    if indx < 0:
                        window_word += start
                    elif indx >= len(sentence):
                        window_word += end
                    else:
                        window_word += sentence[indx]
            windowed_data.append((window_word, labels[i]))
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        embedding = tf.reshape(embedding, [-1,self.config.n_window_features*self.config.embed_size])
    ...
    def add_prediction_op(self):
        ...
        W = tf.get_variable('W',[self.config.n_window_features*self.config.embed_size,
                                 self.config.hidden_size],
                            initializer=tf.contrib.layers.xavier_initializer(seed=1.0))
        U = tf.get_variable('U',[self.config.hidden_size, self.config.n_classes],
                            initializer=tf.contrib.layers.xavier_initializer(seed=4.0))
    ...
    def add_training_op(self, loss):
        ...
        optimizer = tf.train.AdadeltaOptimizer(self.config.lr)
    ...
    def __init__(self, helper, config, pretrained_embeddings, report=None):
        super().__init__(helper, config, report)
