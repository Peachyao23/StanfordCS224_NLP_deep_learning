...
class GRUCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            tf.get_variable('W_r',[self.input_size, self.state_size],
                            initializer=tf.contrib.layers.xavier_initializer())
            tf.get_variable('U_r',[self.state_size, self.state_size],
                            initializer=tf.contrib.layers.xavier_initializer())
            tf.get_variable('b_r', [self.state_size, ], initializer=tf.zeros_initializer())
            tf.get_variable('W_z',[self.input_size, self.state_size],
                            initializer=tf.contrib.layers.xavier_initializer())
            tf.get_variable('U_z',[self.state_size, self.state_size],
                            initializer=tf.contrib.layers.xavier_initializer())
            tf.get_variable('b_z', [self.state_size, ], initializer=tf.zeros_initializer())
            tf.get_variable('W_o',[self.input_size, self.state_size],
                            initializer=tf.contrib.layers.xavier_initializer())
            tf.get_variable('U_o',[self.state_size, self.state_size],
                            initializer=tf.contrib.layers.xavier_initializer())
            tf.get_variable('b_o', [self.state_size, ], initializer=tf.zeros_initializer())
